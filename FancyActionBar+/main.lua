...
---------------------------[   Local Variables   ]-----------------------------
-------------------------------------------------------------------------------
---@class SV : FancyActionBarSV
local SV                                 -- saved variables (accountwide)
---@class CV : FancyActionBarSV
...
function FancyActionBar.GetFixedDuration(id)
  local duration = 0
  local debuff = FancyActionBar.debuffs[id]
  if debuff then
    if debuff.activeOnTarget == true then
      if FancyActionBar.fixedTimers[id] then
        duration = FancyActionBar.fixedTimers[id]
      end
    end
  end
  return duration
...
function FancyActionBar.SetTrapDuration(id, endTime)
  FancyActionBar.fixedTimers[id] = endTime
end

function FancyActionBar.GetTrapDuration(id)
  local duration = 0
  local debuff = FancyActionBar.debuffs[id]
  if debuff then
    if debuff.activeOnTarget == true then
...
function FancyActionBar.FadeEffect(effect) -- reset effect variables and make sure overlay is cleared
  ...
  if effect.slot1 then
    ResetOverlayDuration(FancyActionBar.overlays[effect.slot1])
    effect.slot1 = nil
  end
  if effect.slot2 then
    ResetOverlayDuration(FancyActionBar.overlays[effect.slot2])
    effect.slot2 = nil
  end
  -- effect.faded = true
...
function FancyActionBar.UpdateTaunt(index)
  if FancyActionBar.tauntSlots[index] ~= nil then UpdateOverlay(index) end
end
...
function SlotEffect(index, abilityId, overrideRank, casterUnitTag) -- assign effect and instructions to overlay index.
  ...
  if cfg ~= nil then
    if ignore then
      ...
      if (FancyActionBar.soulTrap[abilityId]) then
        ...
        then
          effectId = ResolveIdToTrack(1, abilityId)
        else
  ...
  if effect then
    ...
    if (change == EFFECT_RESULT_GAINED or change == EFFECT_RESULT_UPDATED and buffType ~= BUFF_EFFECT_TYPE_DEBUFF) then
      if beginTime == endTime then
        UpdatePassiveEffect(abilityId, true)
        return
      end
...
function CreateQuickSlotOverlay(index) -- create quickslot button overlay.
...
function ApplyQuickSlotAndUltimateStyle() -- make sure UI is adjusted to settings
  ...
  if hideUltNumber then SetSetting(SETTING_TYPE_UI, UI_SETTING_ULTIMATE_NUMBER, "false") end
  ...
  if (change == EFFECT_RESULT_GAINED or change == EFFECT_RESULT_UPDATED) then
    ...
    elseif (id == FancyActionBar.tauntId) then
      ...
      if FancyActionBar.lastTaunt ~= nil and FancyActionBar.overlays[FancyActionBar.lastTaunt] ~= nil then
        ...
        UpdateOverlay(FancyActionBar.lastTaunt)
      end
      -- d("Overlay " .. FancyActionBar.activeTaunts[unitId].overlay .. " on " .. unitId .. " for " .. strformat('%0.1fs', endTime - beginTime))
    ...
    elseif (id == FancyActionBar.tauntId) then
      ...
      if FancyActionBar.activeTaunts[unitId] ~= nil then
        if (FancyActionBar.tauntSlots[FancyActionBar.activeTaunts[unitId].overlay] ~= nil and FancyActionBar.tauntSlots[FancyActionBar.activeTaunts[unitId].overlay].unit == unitId) then
          ...
          UpdateOverlay(FancyActionBar.activeTaunts[unitId].overlay)
          -- d("Overlay " .. FancyActionBar.activeTaunts[unitId].overlay .. " on " .. unitId .. " faded")
        end
...
local function Initialize()
  ...
  local function OnEffectChanged(eventCode, change, effectSlot, effectName, unitTag, beginTime, endTime, stackCount, iconName, buffType, effectType, abilityType, statusEffectType, unitName, unitId, abilityId, sourceType)
    ...
    if effect then
      if effect.toggled then -- update the highlight of toggled abilities.
        if change == EFFECT_RESULT_FADED
        then
          UpdateToggledAbility(abilityId, false)
        else
          UpdateToggledAbility(abilityId, true)
        end
        return
      ...
      elseif (change == EFFECT_RESULT_FADED) then
        ...
        if FancyActionBar.activeCasts[effect.id] then
          ...
          if (abilityId == 46327 and FancyActionBar.effects[46327]) or (FancyActionBar.activeCasts[effect.id].begin < (t - 0.7)) then
            ...
            -- d('Fading ' .. effectName .. ': ' .. string.format(t - FancyActionBar.activeCasts[effect.id].begin) .. ' / ' .. tostring(effectType))
  ...
  local function Update()
    ...
    for i, overlay in pairs(FancyActionBar.overlays) do UpdateOverlay(i) end
  end
...
--[[ Retired
  ...
  -- Any effect duration gained.
    ...
    d('hey ' .. actionSlotIndex)
		local effect = FancyActionBar.effects[GetSlotBoundId(actionSlotIndex, hotbarCategory)]
    ...
    FancyActionBar:dbg('Effect Updated: [<<1>>]: <<2>> (<<3>>). <<4>> / <<5>>', actionSlotIndex, GetAbilityName(effect.id), effect.id or 'nil', effect.endTime or 'nil', duration or 'nil')
  end
]]
